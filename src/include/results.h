...
#define CHK_RES(val) do {           \
        if (val != WK_OK) {         \
            res = val;              \
            printf("[CHK_RES] failed at file: %s, func: %s, line: %d, res = %08x", __FILE__, __FUNCTION__, __LINE__, val); \
            goto error_exit;        \
        }                           \
    } while(0)
...
#define CHK_BOOL(val) do {          \
        if (!(val)) {                 \
            printf("[CHK_BOOL] failed at file: %s, func: %s, line: %d, val = %d", __FILE__, __FUNCTION__, __LINE__, val); \
...
#define CHK_NULL(val, error_code) do {         \
        if (val == NULL) {         \
            printf("[CHK_RES] failed at file: %s, func: %s, line: %d, res = %08x", __FILE__, __FUNCTION__, __LINE__, error_code); \
            res = error_code;      \
            goto error_exit;       \
        }                          \
    } while(0)
...
#define CHK_VAL(val) do {           \
        if (val != WK_OK) {         \
            printf("[CHK_VAL] failed at file: %s, func: %s, line: %d, res = %08x", __FILE__, __FUNCTION__, __LINE__, val); \
        }                           \
    } while(0)
