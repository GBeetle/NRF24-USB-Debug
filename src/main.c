...
int main(int argc, char* argv[])
{
    ...
    for (size_t idx = 0; idx < count; ++idx) {
        ...
        rc = libusb_get_device_descriptor(device, (int*)&desc);
        assert(rc == 0);
    ...
    CHK_EXIT(rf24_init(&radio));
    
    /*
    while (1) {
        //ch341a_gpio_setbits(0x02);
        //radio.ce(&radio, RF_LOW);
        radio.csn(&radio, RF_LOW);
        //ch341a_gpio_setbits(0x00);
        //radio.ce(&radio, RF_HIGH);
        radio.csn(&radio, RF_HIGH);
        Sleep(100);
    }
    */
