...
struct libusb_device_handle *esp32_cdc_handle = NULL;

const struct dev_entry devs_esp32_cdc[] = {
    {0x303A, 0x4001, "ESP32", "CDC"},
    {0},
};
...
static void print_hex(const void *buf, size_t len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (i % ESP32_CDC_PACKET_LENGTH == ESP32_CDC_PACKET_LENGTH - 1)
            printf("\n");
    }
}
#endif
...
static int32_t usb_transfer(const char *func, unsigned int writecnt, unsigned int readcnt, const uint8_t *writearr, uint8_t *readarr)
{
    ...
    do {
        ...
        while ((in_done + in_active) < readcnt && state_in[free_idx] == TRANS_IDLE) {
            unsigned int cur_todo = min(ESP32_CDC_PACKET_LENGTH - 1, readcnt - in_done - in_active);
...
int esp32_config_stream(unsigned int speed)
{
    if (esp32_cdc_handle == NULL)
        return -1;
...
int esp32_cdc_send_command(unsigned int writecnt, unsigned int readcnt, const unsigned char *writearr, unsigned char *readarr)
{
    ...
    unsigned int p;
    for (p = 0; p < packets; p++) {
        unsigned int write_now = min(ESP32_CDC_PACKET_LENGTH - 1, write_left);
        unsigned int read_now = min ((ESP32_CDC_PACKET_LENGTH - 1) - write_now, read_left);
        ptr = wbuf[p + 1];
        *ptr++ = 0x01;
        unsigned int i;
    ...
    ret = usb_transfer(__func__, ESP32_CDC_PACKET_LENGTH + packets + writecnt + readcnt,
...
int esp32_cdc_shutdown(void)
{
    ...
    libusb_release_interface(esp32_cdc_handle, 0);
    libusb_close(esp32_cdc_handle);
    libusb_exit(NULL);
    esp32_cdc_handle = NULL;
    return 0;
}
...
{
    if (esp32_cdc_handle != NULL) {
        printf("%s: esp32_cdc_handle already set!\n", __func__);
        return -1;
    }
...
#endif
    uint16_t vid = devs_esp32_cdc[0].vendor_id;
    uint16_t pid = devs_esp32_cdc[0].device_id;
    esp32_cdc_handle = libusb_open_device_with_vid_pid(NULL, vid, pid);
    if (esp32_cdc_handle == NULL) {
        printf("Couldn't open device %04x:%04x.\n", vid, pid);
        return -1;
    }
    printf("Found programmer device: %s - %s\n", devs_esp32_cdc[0].vendor_name, devs_esp32_cdc[0].device_name);
    ...
    ret = libusb_detach_kernel_driver(esp32_cdc_handle, 0);
    ...
    ret = libusb_claim_interface(esp32_cdc_handle, 0);
    if (ret != 0) {
    ...
    if (!(dev = libusb_get_device(esp32_cdc_handle))) {
        printf("Failed to get device from device esp32_cdc_handle.\n");
        goto close_handle;
    }
    ...
    libusb_fill_bulk_transfer(transfer_out, esp32_cdc_handle, WRITE_EP, NULL, 0, cb_out, NULL, USB_TIMEOUT);
    for (i = 0; i < USB_IN_TRANSFERS; i++)
        libusb_fill_bulk_transfer(transfer_ins[i], esp32_cdc_handle, READ_EP, NULL, 0, cb_in, NULL, USB_TIMEOUT);
    
    if ((esp32_config_stream(0X03) < 0))
        goto dealloc_transfers;
