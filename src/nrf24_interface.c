...
{
    if (mode)
        //nrf24->ch341a_iomap = 0X01;
        nrf24->ch341a_iomap |= CS0_ENABLE;
    else
        //nrf24->ch341a_iomap = 0X00;
...
static void printPrettyDetails(struct rf24 *nrf24)
{
    ...
    if (autoAck == 0x3F || autoAck == 0)
    {
        ...
        printf(PSTR("Auto Acknowledgment\t" PRIPSTR
                      "\r\n"),
    ...
    else
    {
        ...
        printf(PSTR("Auto Acknowledgment\t= 0b%c%c%c%c%c%c\r\n"),
...
static WK_RESULT writeBlocking(struct rf24 *nrf24, const void *buf, uint8_t len, uint32_t timeout)
{
    ...
    time_t current;
    time(&current);
    uint32_t timer = current; // Get the time that the payload transmission started
